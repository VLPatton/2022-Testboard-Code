/******************************************************************************
    ...
    Copyright 2022 FIRST Team 3284 - Camdenton LASER Robotics.
******************************************************************************/
...
CSparkMotion::CSparkMotion(int nDeviceID)
{
    ...
    m_pMotor->GetForwardLimitSwitch(SparkMaxLimitSwitch::LimitSwitchPolarity::kNormallyOpen).EnableLimitSwitch(false);	
    m_pMotor->GetReverseLimitSwitch(SparkMaxLimitSwitch::LimitSwitchPolarity::kNormallyOpen).EnableLimitSwitch(false);	
    ...
    switch(m_nCurrentState)
    {
        ...
        case eHomingReverse :
                ...
                ((m_dMaxHomingTime > 0.000) && ((double)m_pTimer->Get() > (m_dHomingStartTime + m_dMaxHomingTime))))
            {
                ...
                m_pPIDController->SetReference(0, ControlType::kVoltage);
                if (m_bBackOffHome)
                {
            ...
            else
            {
                ...
                m_pPIDController->SetReference(m_dRevHomeSpeed, ControlType::kDutyCycle);
            }
            break;
        ...
        case eHomingForward :
            ...
            {
                // Stop the motor and change the control mode to position.
                m_pPIDController->SetReference(0, ControlType::kPosition);
                // Set flag that homing is complete.
                m_bHomingComplete = true;
            ...
            else
            {
                ...
                m_pPIDController->SetReference(m_dFwdHomeSpeed, ControlType::kDutyCycle);
            }
            break;
        ...
        case eManualForward :
            if (!IsFwdLimitSwitchPressed())
            {
                ...
                m_pPIDController->SetReference(m_dFwdMoveSpeed, ControlType::kDutyCycle);
                m_bReady = false;
            }
            else
            {
        ...
        case eManualReverse :
            if (!IsRevLimitSwitchPressed())
            {
                ...
                m_pPIDController->SetReference(m_dRevMoveSpeed, ControlType::kDutyCycle);
                m_bReady = false;
            }
            else
            {
...
void CSparkMotion::SetSetpoint(double dSetpoint, bool bUsePosition)
{
    ...
    if (bUsePosition)
    {
        ...
        if (m_bMotionMagic)
        {
            m_pPIDController->SetReference(dSetpoint * m_dRevsPerUnit * m_nPulsesPerRev, ControlType::kSmartMotion);
        }
        else
        {
            m_pPIDController->SetReference(dSetpoint * m_dRevsPerUnit * m_nPulsesPerRev, ControlType::kPosition);
        }
    }
    else
    {
        ...
        if (m_bMotionMagic)
        {
            m_pPIDController->SetReference(dSetpoint * (84 /8 * m_nPulsesPerRev), ControlType::kSmartVelocity);
        }
        else
        {
            m_pPIDController->SetReference(dSetpoint * (84 /8 * m_nPulsesPerRev), ControlType::kVelocity);
        }
    }
...
void CSparkMotion::StartHoming()
{
    ...
    m_pPIDController->SetReference(0, ControlType::kDutyCycle);
